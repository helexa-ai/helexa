...
// Both `.instrument()` call sites live inside `#[cfg(feature = "cuda")]`
// blocks (the worker-path stream spawn and `inference_tp_stream`), so an
// ungated import warns on the CPU build while an absent one breaks the
// CUDA build — which only branch CI compiles.
#[cfg(feature = "cuda")]
...
impl CandleHarness {
    ...
    ///
    /// Runs the *same* producer as the streaming path and buffers the
    /// result (#285). This used to be a second, independent inference
    /// path holding the per-model `inference_lock` for the whole
    /// request, which made it invisible to the batch engine (#98):
    /// concurrent non-streaming callers serialized completely —
    /// measured on beast at 1.00x aggregate throughput across 1/2/4/8
    /// concurrency, against 3.98x for the same work streamed.
    /// `max_in_flight` bought those requests admission and nothing
    /// else, and nothing distinguished it from a legitimately busy node.
    ///
    /// Collapsing both shapes onto `inference_stream` means batching,
    /// prefix-cache reuse, tool-call extraction, the reasoning governor
    /// and server-measured timing apply identically to both, and there
    /// is no second place the same wire object is assembled — the shape
    /// of defect that #252 and #272 both were.
    pub async fn chat_completion(
        ...
        .await
        .map_err(|e| InferenceError::Other(anyhow::anyhow!(e)))
    }
